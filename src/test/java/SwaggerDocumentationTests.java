...
class SwaggerDocumentationTests {
	...
	void documentsEveryProductionApiWithExpectedSecurityAndResponses() throws Exception {
		mockMvc.perform(get("/v3/api-docs"))
			...
			.andExpect(jsonPath("$.paths.length()").value(44))
			...
			.andExpect(jsonPath("$.paths['/api/auth/local/login'].post.summary").value("심사용 ID/PW 로그인"))
			.andExpect(jsonPath("$.paths['/api/auth/local/login'].post.security").doesNotExist())
			...
			.andExpect(jsonPath("$.paths['/api/members/me/onboarding/complete'].post.summary").value("온보딩 완료"))
			...
			.andExpect(jsonPath("$.paths['/api/v1/ai-analysis'].get.summary").value("AI 루틴 분석"))
			.andExpect(jsonPath("$.paths['/api/v1/ai-analysis'].get.security[0].bearerAuth").isArray())
			.andExpect(jsonPath("$.paths['/api/v1/routine-improvements'].get.summary").value("내 루틴 개선 방향 제시"))
			.andExpect(jsonPath("$.paths['/api/v1/routine-improvements'].get.security[0].bearerAuth").isArray())
			.andExpect(jsonPath("$.paths['/api/v1/routines/daily'].get.summary").value("오늘의 데일리 루틴 조회"))
			.andExpect(jsonPath("$.paths['/api/v1/routines/daily'].get.security[0].bearerAuth").isArray())
			.andExpect(jsonPath("$.paths['/api/v1/routine-logs/today/steps/{stepId}'].patch.summary").value("데일리 루틴 스텝 완료 토글"))
			.andExpect(jsonPath("$.paths['/api/v1/routines/generate'].post.summary").value("AI 자동생성 루틴 미리보기"))
			.andExpect(jsonPath("$.paths['/api/v1/routines/generate'].post.security[0].bearerAuth").isArray())
			.andExpect(jsonPath("$.paths['/api/v1/home'].get.summary").value("홈 요약 조회"))
			.andExpect(jsonPath("$.paths['/api/v1/home'].get.security[0].bearerAuth").isArray())
			.andExpect(jsonPath("$.paths['/api/v1/home/condition'].post.summary").value("오늘의 컨디션 기록/취소"))
