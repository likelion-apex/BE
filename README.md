...
브라우저에서 `http://localhost:3000`을 엽니다. 카카오 개발자 콘솔, 서버 `KAKAO_REDIRECT_URI`, 로컬 `VITE_KAKAO_REDIRECT_URI`는 모두 다음 주소로 설정합니다.

```text
http://localhost:3000/onboarding/kakaocallback
```

배포 서버의 `CORS_ALLOWED_ORIGINS`에도 `http://localhost:3000`이 포함되어야 합니다. 상세한 환경변수와 실행 방법은 [`tools/kakao-login-test-frontend/README.md`](tools/kakao-login-test-frontend/README.md)를 참고하세요.
