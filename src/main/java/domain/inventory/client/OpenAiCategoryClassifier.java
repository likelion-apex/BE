 ...
 *
 * 1) AI 응답 성공 → AI가 제품명을 보고 9종의 실제 스킨케어 단계 카테고리 중 하나가 명확히 맞으면
 *    그것을, 9종에 전혀 해당하지 않으면(선크림/클렌저/마스크 등) ETC(기타)를 스스로 판단해서 반환
 * 2) API 장애/파싱 실패 등 응답 자체를 못 받은 경우 → null(카테고리 미지정)을 반환
 */
@Slf4j
@Component
public class OpenAiCategoryClassifier {

    /** API 장애 등으로 분류 자체가 불가능할 때만 쓰는 고정 기본값. 10종 중 어느 것도 강제하지 않고 null로 둔다. */
    private static final ProductCategory DEFAULT_CATEGORY = null;

    private static final String SYSTEM_PROMPT = """
            당신은 화장품을 카테고리로 분류하는 어시스턴트입니다.
            사용자가 알려준 화장품 제품명을 보고 다음 10개 카테고리 중 가장 가까운 것 하나로 반드시 분류하세요:
            SKIN_TONER(스킨/토너), LOTION(로션/에멀전), ESSENCE_SERUM(에센스/앰플/세럼), FACEOIL(페이스오일),
            CREAM(크림), EYECARE(아이케어), MIST(미스트,젤), SKIN_TONERPAD(스킨/토너 패드), BAM(밤/멀티밤),
            ETC(기타: 선크림, 클렌저, 마스크팩 등 위 9개 스킨케어 단계 카테고리에 전혀 해당하지 않는 제품)
            위 9개 중 하나가 명확히 가장 가까우면 그것을 선택하고, 9개 중 어디에도 해당하지 않으면 ETC를 선택하세요.
            null이나 그 외 값은 절대 반환하지 마세요.
    ...
    ProductCategory parseCategory(JsonNode response, String productName) {
        if (response == null) {
            return DEFAULT_CATEGORY;
        }
