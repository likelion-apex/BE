...
public class OpenAiIngredientClient {
    ...
    public static final String DETAIL_SYSTEM_PROMPT = """
            당신은 화장품 성분의 배합목적, 피부 효능, 위험도를 알려주는 어시스턴트입니다.
            사용자가 알려준 성분 목록 각각에 대해 다음을 판단하세요:
            1. 배합목적(purposes): 아래 7개 공식 카테고리 중 해당되는 것을 모두 골라 나열하세요.
               - 용제, 피부컨디셔닝제, 헤어컨디셔닝제, 피부보호제, 착향제, 변성제, 향료
               위 7개 카테고리 이름 그대로만 사용하고, 이 외의 표현은 사용하지 마세요.
               완전히 확신할 수 없어도 화장품 성분 지식을 바탕으로 가장 가능성 높은 카테고리를
               최소 1개는 반드시 선택하세요(빈 배열 금지).
            2. 효능(efficacyTags): "피부 보습", "피부 보호" 중 해당되는 것을 0~2개 선택하세요.
               둘 다 해당 없으면 빈 배열로 응답하세요(강제로 채우지 마세요).
            3. 위험도(riskLevel): 피부 자극/알레르기 유발 가능성 등을 고려한 일반적인 위험도를
               "LOW"(낮음, 안전), "MEDIUM"(중간, 주의 필요), "HIGH"(높음, 자극/위험 가능성 높음)
               중 하나로 판단하세요. 확신이 없으면 "MEDIUM"으로 응답하세요.
            반드시 아래 JSON 형식으로만, 요청받은 성분 개수와 이름을 그대로 유지하여 답변하세요:
            {"ingredients": [{"name": "성분1", "purposes": ["용제"], "efficacyTags": ["피부 보습"], "riskLevel": "LOW"}]}
            """;
